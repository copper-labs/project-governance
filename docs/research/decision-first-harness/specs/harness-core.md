...
2. **Every question has a deterministic fallback.** When the decision provider is unavailable,
   invalid, or below threshold, the loop takes the conservative answer and continues.
...
- Every decision that influenced an action is recorded before that action runs.
- A missing, malformed, or low-confidence decision is a fallback, never a silent default.
...
- Which repository owns this family once it leaves research.
- Host language for the CLI, given a Python governance runtime and TypeScript and KMP adopters.
