...
export function readCheckRecord(path: string): Record<string, unknown> | null {
  ...
  try {
    ...
    if (!before.isFile() || before.size > 16 * 1024 * 1024) throw new Error("Invalid run record");
    // Durable writers replace the path by rename. The opened, stable inode is a valid snapshot.
    const bytes = readFileSync(fd, "utf8"), after = fstatSync(fd);
    if (before.size !== after.size || before.mtimeMs !== after.mtimeMs)
      throw new Error("Run record changed during read");
...
export function inspectCheckRun(id: string, root = checkRunRoot()) {
  ...
  const directory = join(root, id), intent = readCheckRecord(join(directory, "run.json"));
  ...
  for (const pack of order as string[]) {
    ...
    for (const name of names.filter(name => /^command-[0-9]+$/u.test(name)).sort()) {
      const commandDirectory = join(packDirectory, name), request = readCheckRecord(join(commandDirectory, "request.json"));
