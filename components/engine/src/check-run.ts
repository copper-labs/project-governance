...
export async function runChecks(packs: Packs, plan: ValidationPlan, request: Omit<BuiltinCheckRequest, "id">, options: { root?: string; deadlineMs?: number; reservedRunId?: string; } & CheckObservationContext = {}) {
  ...
  try {
    ...
    const result = await executeChecks(packs, plan, request, undefined, async (entry, packId, index) => {
      ...
      return runNativeCheckCommand({ directory: join(packDirectory, `command-${index}`), id: `${id}:${packId}:${index}`, root: request.subject.root,
        argv, deadlineMs, cancelled, env: { ...packet.env, PROJECT_GOVERNANCE_RUN_ID: id, PROJECT_GOVERNANCE_EVIDENCE_ROOT: evidence } });
    }, packId => inspectEvidenceManifest(join(directory, digest(packId).slice(7), "evidence"), request.scope.subject_digest, request.assets.schema("evidence-manifest")), cancelled);
    ...
    try { const metric: RunMetric = { version: 1, ...(options.trigger ? { trigger: options.trigger } : {}), ...(options.expectedStatus ? { expected_status: options.expectedStatus } : {}), run_id: id, workspace: request.subject.root, stage: plan.stage, runtime_version: "3.0.0-preview.1",
