...
export async function discoverStartupRelease(profile:unknown,currentValue:unknown,
 ...
 const current=compiledRuntimeLock(currentValue);stableStartupVersion(current.version);
 const match=/^https:\/\/github\.com\/([A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+)\/releases\/download\/([^/]+)\/[^/]+\.tgz$/u.exec(current.artifact.url);
 if(!match || match[2]!==current.version)throw new Error("Startup discovery requires the configured GitHub release owner");
 ...
 const result=await discover();cache?.commit();return result;
}
