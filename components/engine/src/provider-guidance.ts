...
Before task-specific source or documentation reads, use the governance packet supplied by the prompt hook. If no packet arrived, run \`doctor --capability context\` and \`context-route\`; do not assume installed hooks executed. When binding reports \`refresh-required\` or scope changes, run \`context-route --task <current request>\` before more task-specific reads; a recorded task switch retains the turn's shared allowance. Expand from original references when needed, and record packet misses with \`telemetry context expansion --entry <id> --path <path>\`. Normal checks and Git hooks inherit the binding. A provisional prompt entry is not task acceptance; a startup child cannot set its parent's environment.
