...
test("provider telemetry binds receipts, preserves native accounting and never scans private logs", () => {
  ...
  try {
    const jobs = ["claude", "codex", "gemini"].map(provider => {
      const directory = join(root, provider), request = { version: 1, provider: { kind: provider }, prompt: "private-canary" }, requestDigest = digest(request);
      durableJson(join(directory, "request.json"), request);
      const providerResult = join(directory, "provider-result.json");
      const usage = provider === "claude" ? { usage: { input_tokens: 100, output_tokens: 20 }, estimated_cost_usd: 0.01, secret: "private-canary" }
