...
import { testTmpdir as tmpdir } from "./helpers.ts";
...
test("only instruction files already present are touched", () => {
    ...
    const results = initRepo(dir);
    assert.deepEqual(results.map((r) => r.file).sort(), ["AGENTS.md", "CLAUDE.md"]);
    assert.equal(existsSync(join(dir, "CODEX.md")), false, "a missing host file is not created");
});
test("the block routes, and carries no policy or thresholds", () => {
    ...
    assert.ok(text.includes("never edits product source"), "and that the host still edits");
    assert.ok(!/threshold|confidence/i.test(text), "thin: no classification policy in the adapter");
});
