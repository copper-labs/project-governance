...
4. For non-trivial implementation, use the delegated-execution skill to classify each packet-ready
   slice and record its capability tier before routing.
5. Declare a `Proof Budget`: claims to prove, cheapest sufficient evidence, normally one complete
   proof cycle, expected duration or cost, invalidation rules, equivalent-repeat reason policy, and
   an explicit stop condition.
6. Map changed areas to validation packs, review skills, rollback, and evidence. Treat checks that
   prove the same claim on the same digest-bound subject as substitutes, not additive ceremonies.
7. Record open questions only when they block safe execution.
8. Keep the plan in the correct lifecycle location and link every active plan from
...
Report the plan path or no-plan rationale, dependency order, packet-ready slices, capability tiers,
validation packs, proof budget and stop condition, review skills, rollback, and unresolved blockers.
