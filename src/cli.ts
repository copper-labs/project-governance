...
function main(argv: string[]): void {
    ...
    try {
        ...
        if (group === "governance" && verb === "plan")
            return emit(governancePlan(store, needTask().taskId, root, one("executor") ?? join(root, ".governance/runtime/bin/project-governance"), one("stage") ?? "pre-commit", !!flags["staged"]));
        if (group === "check" && verb === "run") {
            ...
            const owner = new GovernanceExecutor(one("executor") ?? join(root, ".governance/runtime/bin/harness-agent"), one("state-root"));
