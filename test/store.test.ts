...
test("revoking a constraint is explicit and never a side effect", () => {
  ...
  const dropped = s.reviseTask(t.taskId, [], { revoke: [0] });
...
test("provenance kinds are preserved, not merged", () => {
  ...
  assert.deepEqual(
    t.items.map((i) => i.provenance),
    ["operator", "hypothesis", "observed"],
  );
  s.close();
});
...
test("two callers racing a transition: the second does not act", () => {
  ...
  assert.throws(
    () => s.transitionAction(a.actionId, a.revision, "authorized"),
    (err: unknown) => err instanceof RevisionConflict,
    "the stale expected revision is refused",
  );
...
test("analytics loss never blocks; execution-critical loss is raised", () => {
  ...
  assert.throws(
    () => s.recordEvidence({ ...base, criticality: "execution" }),
    (err: unknown) => err instanceof ExecutionStateUnavailable,
  );
});
