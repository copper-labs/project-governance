...
import { resolveSubject, readAtSubject, retrieve } from "../src/ops/retrieval.ts";
...
test("caller-supplied budget accounting preserves optional omissions", () => {
    const dir = repo();
    const store = new Store(":memory:");
    const subject = resolveSubject(dir, { kind: "commit", rev: "HEAD" });
